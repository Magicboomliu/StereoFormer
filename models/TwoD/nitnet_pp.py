...
class EDNet(nn.Module):
    def __init__(self, batchNorm=False, max_disp=192, input_channel=3, res_type='normal', squeezed_volume=False):
        ...
        self.conv_offset = ResBlock(512, 24)
    ...
    def forward(self, img_left, img_right, training=False, disp=None):
        ...
        # build corr
        offset = self.conv_offset(torch.cat((conv3_l, conv3_r), dim=1))
        out_corr = form_dynamic_cost_volume(offset, conv3_l, conv3_r)
        out_conv3a_redir = self.conv_redir(conv3_l)
        in_conv3b = torch.cat((out_conv3a_redir, out_corr), dim=1)
