...
from models.IterativeLocalCostVolume.utils.utils import bilinear_sampler
...
class CorrBlock1D:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        ...
        corr = CorrBlock1D.corr(fmap1, fmap2) #[B,H,W1,1,W2]--->1/4 Cost Volume Here
        ...
        # torch.Size([12800, 1, 1, 160]) --> All possible locations ; receptive filed: Simple
        for i in range(self.num_levels):
            corr = F.avg_pool2d(corr, [1,2], stride=[1,2])
            # torch.Size([12800, 1, 1, 80])  --> Downsample one ; Receptive Filed: bigger
            # torch.Size([12800, 1, 1, 40])  --> less Coarse one ; Recepetive Filed : more bigger
            # torch.Size([12800, 1, 1, 20]) --> Coarse one ; Receptive Filed: more and more bigger
            # torch.Size([12800, 1, 1, 10]) --> Most Coarse One; Receptive Filed : Maxium
            self.corr_pyramid.append(corr)
