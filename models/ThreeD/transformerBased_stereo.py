...
class Swin_Stereo(nn.Module):
    def __init__(self,max_disp=192,
        ...
        if self.use_feature_fusion:
            # Feature Fusion
            out_dim = 12
            self.feature_fusion = ConcatHead(hrn_out_dim=1440,out_dim=12)
...
class ConcatHead(nn.Module):
    ...
    def __init__(self,hrn_out_dim,out_dim=12):
        super().__init__()
        self.proj = nn.Sequential(convbn(hrn_out_dim,320,3,1,1,1),
                                 nn.ReLU(inplace=True))
        self.agg_conv = nn.Sequential(convbn(320, 128, 3, 1, 1, 1),
                                          nn.ReLU(inplace=True),
    ...
    def forward(self, x_list):
        ...
        upsample_out_gwc = self.proj(upsample_out_gwc)
...
if __name__=="__main__":
    ...
    swin_stereo = Swin_Stereo(pretrain=False,max_disp=192,use_feature_fusion=True,use_concated_volume=True).cuda()
    
    disparity_pyramid = swin_stereo(left_input,right_input,True)
