...
from models.LocalCostVolume.Attempts.variance_local_cost_volume import Variance_Based_Local_Cost_Volume
...
class LowCNN(nn.Module):
    def __init__(self, max_disp=192,cost_volume_type='correlation',
        ...
        if self.adaptive_refinement:
            self.local_cost_volume = Variance_Based_Local_Cost_Volume(garma=1,sample_points=20)
    ...
    def forward(self,left,right,is_training=True):
        ...
        if self.adaptive_refinement:
            ...
            new_disp = self.local_cost_volume(final_cost,low_scale_disp3,True)
