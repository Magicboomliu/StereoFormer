...
if __name__=="__main__":
    ...
    pr3 = torch.abs(torch.randn(1,1,320,640)).cuda()
    pr2 = torch.abs(torch.randn(1,1,320,640)).cuda()
    pr1 = torch.abs(torch.randn(1,1,320,640)).cuda()
    pr0 = torch.abs(torch.randn(1,1,320,640)).cuda()
    
    left_image = torch.randn(1,3,320,640).cuda()
    right_image = torch.randn(1,3,320,640).cuda()
    
    weight_list = [1.0,0.8,0.8,0.6]
    multi_loss_critizon = multiscaleloss(scales=4,downscale=1,weights=weight_list,loss='Smooth_l1')
    
    loss = multi_loss_critizon([pr3,pr2,pr1,pr0],gt_disp)
