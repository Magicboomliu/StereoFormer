...
from models.TwoD.Test_Adaptive_Cost_Volume import LowCNN
...
class DisparityTrainer(object):
    def __init__(self, lr, devices, dataset, trainlist, vallist, datapath, 
        ...
        if model =='LowCNN_simple':
            self.total_loss = TotalLoss(disp_mode='Smooth_l1',alpha=0.5,disp_emphasis=3,disp_only=True)
        elif model =='LowCNN_ada':
            self.total_loss = TotalLoss(disp_mode='Smooth_l1',alpha=0.9,disp_emphasis=3,disp_only=False)
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            if self.model =='LowCNN_simple':
                ...
                output,lower_bound,upper_bound,disp3 = self.net(left_input, right_input,True)
                loss = self.total_loss(output,target_disp,lower_bound,upper_bound,disp3)
                
                print("Lower Bound: ",lower_bound.mean())
                print("Higher Bound: ",upper_bound.mean())
        ...
        for i, sample_batched in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                if self.model=='LowCNN_simple':
                    ...
                    output,lower_bound,upper_bound,disp3 = self.net(left_input,right_input,False)
