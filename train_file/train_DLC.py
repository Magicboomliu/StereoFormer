...
from losses.multi_equal_with_supervised import multiLossWithRangeLoss
...
class DisparityTrainer(object):
    def __init__(self, lr, devices, dataset, trainlist, vallist, datapath, 
        ...
        if model =='LowCNN_simple':
            ...
            self.total_loss = multiLossWithRangeLoss(gamma=0.9,weight=[0.8,1.2])
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            if self.model =='LowCNN_simple':
                ...
                output_list, bounds,disp3= self.net(left_input, right_input,True)
                loss = self.total_loss(output_list,target_disp,disp3,bounds[0],bounds[1])
        ...
        for i, sample_batched in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                if self.model=='LowCNN_simple':
                    ...
                    output_list,bounds,disp3 = self.net(left_input,right_input,False)
                    output = output_list[-1]
