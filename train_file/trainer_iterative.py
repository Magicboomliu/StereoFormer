...
from models.RAFT_Stereo.raft_stereo import RAFTStereo,Args
...
class DisparityTrainer(object):
    ...
    def _build_net(self):
        ...
        if self.model == 'RAFT':
            self.net = RAFTStereo(args=modelargs)
        else:
        ...
        if self.pretrain == 'none':
            ...
            # pretrain_ckpt = "/home/zliu/Desktop/target.pth"
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            # Inference Here
            output_list = self.net(left_input, right_input,test_mode=False)
            ...
            if type(loss) is list or type(loss) is tuple:
                loss = np.sum(loss)
            if type(output_list) is list or type(output_list) is tuple:
                
                flow2_EPE = self.epe(output_list[-1], target_disp)
    ...
    def validate(self,summary_writer,epoch,vis=False):
        ...
        for i, sample_batched in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                outputs = self.net(left_input, right_input,test_mode=True)
                output = outputs[1]
